...
import pb "github.com/hyperledger/awesomeProject/fourmode/grpcguide"
...
// RecordRoute记录了一个由Point组成的Route
...
// 不断的接受RouteNote（某个位置及其消息），将相同位置的点加入routeNotes数组中
// 返回这个数组中的RouteNote，即获取RouteNote后返回相同位置的RouteNote
...
func  main()  {
	...
	// 创建一个routeGuideServer，为其添加savedFeatures
	s := &routeGuideServer{routeNotes: make(map[string][]*pb.RouteNote)}
	if err := json.Unmarshal(exampleData, &s.savedFeatures); err != nil {
		log.Fatalf("Failed to load default features: %v", err)
	}
	pb.RegisterRouteGuideServer(grpcServer, s) // 注册到grpc服务器上，实现了routeGuideServer的方法
