...
peer chaincode install -n r_test_cc6 -v 1.0 -p github.com/hyperledger/fabric/examples/chaincode/go/example01

# 实例化chaincode代码

peer chaincode instantiate -o orderer.cmbc.com:7050 -C cmbcchannel -n r_test_cc6 -v 1.0 -c '{"Args":["init","a","100","b","200"]}' -P "OR ('Org1MSP.member','Org2MSP.member')"

# 调用代码

peer chaincode invoke -o orderer.cmbc.com:7050 -C cmbcchannel -n r_test_cc6 -c '{"Args":["invoke","a","b","1"]}'


# 查询数据

peer chaincode query -C cmbcchannel -n r_test_cc6 -c '{"Args":["query","a"]}'
